...
首次运行自动准备内置 Python、离线安装依赖、初始化数据库；没有启用的超级管理员时，交互提示创建超级管理员，密码至少 12 字符。已有管理员和授权数据会保留。
...
在新目录解压，再将旧配置复制到新目录 `.env`，确认仍连接同一数据库后启动。不要复制旧 `.venv` 或覆盖正在运行的目录。当前数据库升级至版本 6：旧版本补齐管理员角色字段，新增 CLI 调用日志的完整命令字段，以及人员和环境的创建时间、修改时间、最新操作人字段。先备份数据库并停止旧服务，再执行启动器迁移；已有管理账号迁移为超级管理员，其他数据保留。
