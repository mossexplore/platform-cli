...
def test_multi_environment_upsert_and_independent_revoke(system):
    ...
    assert client.post('/admin/grants/batch', data=payload).status_code == 200
    assert client.post('/admin/grants/batch', data=payload).status_code == 200
...
def test_invalid_environment_batch_changes_nothing(system):
    ...
    for names in [['prod', 'missing'], ['prod', 'dev']]:
        assert client.post('/admin/grants/batch', data={'csrf': csrf, 'username': 'alice', 'environments': names}).status_code == 400
        assert check(client).json()['allowed']
    assert client.post('/admin/grants/batch', data={'csrf': csrf, 'username': 'alice'}).status_code == 422
    assert client.post('/admin/grants/batch', data={'csrf': 'bad', 'username': 'alice', 'environments': ['prod']}).status_code == 403
    page = client.get('/admin?tab=grants').text
