...
def test_effective_grant_counts_and_filters(system):
    ...
    assert '授权列表 <span class="count">1 个账号' in client.get('/cli-permission/admin?tab=grants&status=expiring').text
    with app.state.sessions() as db:
        db.scalar(select(User)).enabled = False
        db.commit()
        assert overview(db)['active'] == 0
        assert overview(db)['expiring'] == 0
    assert '授权列表 <span class="count">0 个账号' in client.get('/cli-permission/admin?tab=grants&status=active').text
...
def test_name_and_environment_search(system):
    _, client, _ = system
    login(client)
    assert '人员列表 <span class="count">1 条' in client.get('/cli-permission/admin?tab=users&q=Alice').text
    assert '环境列表 <span class="count">1 条' in client.get('/cli-permission/admin?tab=environments&q=生产').text
    assert '授权列表 <span class="count">1 个账号' in client.get('/cli-permission/admin?tab=grants&q=prod').text
...
def test_expired_grant_and_environment_disabled(system):
    ...
    with app.state.sessions() as db:
        ...
        db.commit()
    assert '已过期' in client.get('/cli-permission/admin?tab=grants&status=expired').text
    with app.state.sessions() as db:
        db.scalar(select(Environment)).enabled = False
        db.commit()
    assert '环境已停用' in client.get('/cli-permission/admin?tab=grants&status=disabled').text


def test_audit_changes_are_readable_and_escaped(system):
    app, client, _ = system
    csrf = login(client)
    client.post('/cli-permission/admin/users', data={'csrf': csrf, 'item_id':1, 'username':'alice', 'display_name':'<script>bad</script>'})
    response = client.get('/cli-permission/admin?tab=audit')
