...
def audit_detail(item):
    ...
    if item.action == 'version_policy.delete':
        return {'label': ACTIONS[item.action],
            'summary': '删除 · ' + str(before.get('name')),
            'changes': [
                {'field': '策略名称', 'before': before.get('name'), 'after': '已删除'},
                {'field': '撤销的临时例外', 'before': str(len(data.get('revoked_exception_ids', []))) + ' 项', 'after': '已撤销'}]}
    if item.action == 'administrators.delete':
        return {'label': ACTIONS[item.action], 'summary': '删除 · ' + str(before.get('username')),
            'changes': [
                {'field': '管理员账号', 'before': before.get('username'), 'after': '已删除'},
                {'field': '姓名', 'before': before.get('display_name') or '—', 'after': '已删除'}]}
