...
管理台“监控与审计 → 数据看板”按调用日志统计授权检查次数、通过与拒绝、活跃用户、时间趋势、常用命令、活跃用户，以及按原因、环境和命令划分的拒绝次数。页面可按时间、环境、业务 ID、用户名、命令名称和 CLI 版本筛选，点击趋势、排行或卡片进入对应调用日志；自定义时间范围最多 90 天，时间按北京时间显示。统计基于 `cli_call_logs` 的原始检查记录，客户端预检查和网关检查分别计数。授权通过不代表业务命令执行成功；未触发在线检查的操作不在统计范围内。看板与调用日志使用相同的管理员登录会话，不向未登录访问者开放。
