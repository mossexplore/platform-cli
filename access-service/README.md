...
管理台“监控与审计 → 数据看板”按调用日志统计授权检查次数、通过与拒绝、活跃用户、常用命令，以及按原因、环境和命令划分的拒绝次数。“数据分析”提供命令分布柱状图、调用趋势折线图、调用次数分布饼图、调用次数排行柱状图；小时、天、周粒度控制时间序列的横坐标，分布和排行按整个查询时段汇总。页面可按开始时间、结束时间、时间粒度、环境、用户名、业务 ID 和 CLI 版本筛选，点击图表、排行或卡片进入对应调用日志。默认最近 7 天，时间范围最长 90 天，按小时分析最长 14 天；时间按北京时间显示。统计基于 `cli_call_logs` 的原始检查记录，客户端预检查和网关检查分别计数。授权通过不代表业务命令执行成功；未触发在线检查的操作不在统计范围内。看板与调用日志使用相同的管理员登录会话，不向未登录访问者开放。
