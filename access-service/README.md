...
1. CLI 沿用现有平台登录，执行业务命令前向权限服务发送当前登录账号 `username`、当前环境、平台源地址、命令名和 `businessid`。
...
`GET /cli-permission/healthz` 检查数据库及结构版本，正常返回 `{"status":"ok"}`。当前结构版本为 3，版本 1 升级时新增 cli_call_logs 表，版本 2 升级时新增管理员 role 字段；保留原有数据，迁移可重复执行；后续数据库结构升级必须新增显式迁移。更新前备份数据库与 `/etc/cli-access/env`，上传新包并重新运行安装；配置保留，服务会短暂停止。
