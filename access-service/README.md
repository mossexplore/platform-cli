...
管理台新增“CLI 版本管理”，支持全局、环境、业务策略，观察/提醒/强制模式、最低/推荐/禁用版本、预约生效、有期限的例外、发布预览及审计。旧客户端缺少版本头时按 **1.0.0**；历史日志也以 1.0.0 标记为“历史默认推定”。升级前备份数据库、停止旧服务并执行 `bash manage.sh migrate`。
