...
首次连接新数据库需要运行迁移，再交互创建管理员。已有数据库结构为版本 8 且管理员正常时，无需执行本节命令；从 1.0.0 的结构 7 升级必须先停写、备份，再使用新镜像运行 migrate，不重新创建管理员。
...
管理员密码交互输入（至少 12 字符），不写在命令行。默认启动命令不会自动迁移或创建管理员。1.0.3 的数据库结构为 8。
...
保留旧镜像标签和对应配置便于回滚。1.0.0 → 1.0.3 涉及结构 7 → 8；新版只新增版本表和日志字段，但旧版健康检查要求结构 7，回滚必须恢复升级前数据库及匹配配置，不能只换镜像或手改结构版本。最终备份应在停止全部写入后完成，恢复业务前核验原数据、授权结果和新日志写入。不要清理仍用于生产或回滚的镜像。
