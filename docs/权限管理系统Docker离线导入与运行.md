...
首次连接新数据库需要运行迁移，再交互创建管理员。已有数据库结构为版本 10 且管理员正常时，无需执行本节命令；从 1.0.3 的结构 8 升级必须先停写、备份，再使用新镜像运行 migrate，不重新创建管理员。
...
管理员密码交互输入（至少 12 字符），不写在命令行。默认启动命令不会自动迁移或创建管理员。1.0.3.1 的数据库结构为 10。
...
下载新附件并校验 → docker load → 在独立库演练备份恢复与迁移 → 停止旧应用和写入 → 完成最终数据库与配置备份 → 单进程使用新镜像运行 migrate → 保留旧容器并以新标签启动 → 验证健康、登录、授权和日志。详细分步操作见 `Docker-upgrade-1.0.3.1.md`。

保留旧镜像标签和对应配置便于回滚。1.0.3 → 1.0.3.1 涉及结构 8 → 10；新版新增管理员姓名和策略删除标记，旧版健康检查要求结构 8，回滚必须恢复升级前数据库及匹配配置，不能只换镜像或手改结构版本。最终备份应在停止全部写入后完成，恢复业务前核验原数据、授权结果和新日志写入。不要清理仍用于生产或回滚的镜像。
