...
权限系统结构由 8 升至 10，CLI 保持 1.0.3。先在独立库演练备份恢复和迁移；维护窗口停写后完成最终备份，再用 1.0.3.1 镜像单进程显式迁移。迁移后核对数据、健康和授权，不能仅切回要求结构 8 的旧镜像。完整命令、验收与回滚条件见 [独立升级指南](权限管理系统1.0.3升级至1.0.3.1指南.md)，正式附件为 `Docker-upgrade-1.0.3.1.md`。
