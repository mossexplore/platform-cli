...
适用版本：项目 **0.3.32**，数据库结构版本 **3**。本文依据当前离线包的启动、安装、配置及迁移代码编写。命令中的服务器 IP、数据库账号、密码和域名须替换为实际值。
...
不要复制旧 `.venv`、`python`、`.portable-ready` 或旧 `SHA256SUMS`。本次结构由 2 升为 3，新增 `admins.role`；原有管理账号保留为超级管理员，原停用状态保持。版本 1 会先补调用日志表再升级。迁移可重复执行，但 MySQL DDL 自动提交，不能假定事务失败会撤销全部结构变更。
...
数据库已升为版本 3 时，不要直接启动只支持版本 2 的旧程序。先停止新服务，由数据库维护人员确认兼容性，或将升级前备份恢复到独立数据库，配合对应旧程序及配置验证后切换。
