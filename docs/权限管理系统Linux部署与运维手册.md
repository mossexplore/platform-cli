...
适用版本：项目 **0.3.41**，数据库结构版本 **7**。本文依据当前离线包的启动、安装、配置及迁移代码编写。命令中的服务器 IP、数据库账号、密码和域名须替换为实际值。
...
不要复制旧 `.venv`、`python`、`.portable-ready` 或旧 `SHA256SUMS`。当前结构版本为 7：版本 6 升级时新增权限申请和提交频率记录表；版本 2 升级时新增 `admins.role`，版本 3 升级时新增 `cli_call_logs.full_command`，版本 4 升级时新增人员和环境的时间及操作人字段；原有管理账号保留为超级管理员，原停用状态保持。版本 1 会先补调用日志表再升级。迁移可重复执行，但 MySQL DDL 自动提交，不能假定事务失败会撤销全部结构变更。
...
用户申请入口为 `/cli-permission/apply`，填写 W3 账号（如 `l00123456`）和姓名。管理员在“权限申请”菜单核对或修正信息，再确认环境授权。默认选择所有启用环境，新授权默认长期有效。详细规则见 [权限服务说明](../access-service/README.md#用户申请与管理员审批)。

本功能需要结构版本 7。先备份并停止旧服务，再运行 `bash manage.sh migrate`，最后启动新服务。使用 Nginx 时同步更新示例中的专属查询链接日志排除配置。
