...
- 使用镜像 `cli-access:1.0.3.1` 启动，默认端口 `8008`。
...
新数据库需要初始化并创建管理员；已有结构版本 10 且管理员正常时无需重新初始化。由权限系统 1.0.3 升级必须完成数据库结构 8 → 10 迁移，步骤见 [升级与回滚指南](docs/权限管理系统1.0.3升级至1.0.3.1指南.md)。
