...
当前版本：**权限管理系统与 CLI 均为 1.0.3.2**。本地开发参见 [Jupyter 本地开发指南](docs/Jupyter本地开发指南.md)，发布内容见 [1.0.3.2 发布说明](docs/RELEASE_NOTES_1.0.3.2.md)。

## 下载 1.0.3.2 正式版本

前往 [GitHub 正式版本](https://github.com/mossexplore/platform-cli/releases/tag/v1.0.3.2)，按需要下载：

| 文件 | 用途 |
| --- | --- |
| `wiserec-cli-1.0.3.2-windows-py3-online.zip` | Windows 联网安装，支持企业 Python 包源 |
| `wiserec-cli-1.0.3.2-windows-x64-py312-offline.zip` | Windows x64、Python 3.12 离线安装 |
| `wiserec_cli-1.0.3.2-py3-none-any.whl` | 通过 pip 安装 CLI，依赖另行准备 |
| `cli-access-1.0.3.2-linux-amd64.tar.gz` | Linux x86_64 权限系统 Docker 镜像 |
...
- 使用镜像 `cli-access:1.0.3.2` 启动，默认端口 `8008`。
...
新数据库需要初始化并创建管理员；已有结构版本 10 且管理员正常时无需重新初始化。由权限系统 1.0.3.1 升级至 1.0.3.2 没有结构迁移，步骤见 [升级与回滚指南](docs/权限管理系统1.0.3.1升级至1.0.3.2指南.md)。更早的结构版本需先按历史指南迁移。
