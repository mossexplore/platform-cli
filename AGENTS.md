...
v1.0.3.1 数据看板曾因 MySQL 聚合结果包含 `Decimal`，在模板的 `chart|tojson` 处抛出 `TypeError: Object of type Decimal is not JSON serializable`，导致页面返回 500。后续修改统计页面或接口时必须遵守：

1. **不要假定数据库聚合结果类型一致**：`COUNT`、`SUM`、`SUM(CASE ...)` 等结果的 Python 类型可能因数据库驱动而不同，`SUM` 在空集上还可能返回 `None`；SQLite 测试通过不代表 MySQL 渲染安全。
2. **在服务端数据组装边界统一类型**：聚合结果进入模板上下文、`tojson`、API 响应或图表配置前，按字段语义转成可序列化的 Python 内建数值类型。调用次数等整数指标转为 `int`，空集聚合的 `None` 按统计口径处理为 `0`；金额等需要小数精度的字段不得盲目转成整数。
3. **不要用字符串化掩盖类型问题**：不能依赖模板直接输出、`json.dumps(default=str)` 或前端强制转换来处理 `Decimal`，以免图表拿到字符串而出现排序、计算或展示错误。
4. **覆盖真实序列化路径**：新增或修改统计查询时，测试需模拟 MySQL 返回 `Decimal` 和空集返回 `None`，实际渲染包含 `tojson` 的页面或序列化 API 响应，并断言图表数据为数值类型。正式发布前还需在 MySQL 环境访问有数据的数据看板，确认页面正常返回且图表 JSON 可解析。
